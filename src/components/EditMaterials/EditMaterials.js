...
const EditMaterials = () => {
    ...
    function handleDeleteBtn(id, name) {
        if (window.confirm(`Вы хотите удалить материал ${name}`)) {
            deleteMaterial(id);
        }
    }
                ...
                <span className="news__title">МАТЕРИАЛЫ ДЛЯ СКАЧИВАНИЯ</span>
                {
                    addingMaterial ?
                        <form onSubmit={(event) => event.preventDefault()} className="add-material__form">
                            ...
                            <label className="add-material__form__label">Раздел материала *</label>
                            <input name="category" onChange={handleInpChanges} value={inpData.category} className="add-material__form__input" type="text" placeholder="Раздел" />
                            ...
                            (
                                materials.materials.map(elem => (
                                    <div key={elem.material_pathname} className="download__item">
                                        ...
                                        <button onClick={() => handleDeleteBtn(elem.material_id, elem.material_name)} className="delete-btn">Удалить <DeleteIcon /></button>
                                    </div>
                                ))
                            )
                            :
